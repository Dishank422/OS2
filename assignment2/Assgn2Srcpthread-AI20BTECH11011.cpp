...
int main() {
    ...
    int task_to_be_assigned = 0;
    while(true)
    {
        // assigning tasks in a round-robin fashion till there are tasks to do
        // First assign task to each thread
        int max_working_thread = 0;
        for(; max_working_thread<n_threads; max_working_thread++) {
            tasks[task_to_be_assigned].thread_num = max_working_thread;
            pthread_create(&my_threads[max_working_thread], nullptr, validator, &tasks[task_to_be_assigned]);
            task_to_be_assigned++;
            if(task_to_be_assigned==3*dim) break;
        } // launching the threads

        // join all threads, then assign tasks again in next iteration of while loop
        for(int i=0; i<= min(max_working_thread, n_threads-1); i++) pthread_join(my_threads[i], nullptr); // joining the threads in a for loop
        if(task_to_be_assigned==3*dim) break; // if no more tasks to assign, exit the while loop
    }
