...
atomic_flag lock = ATOMIC_FLAG_INIT; //atomic lock variable
...
void testCS(int id)
{
    ...
    for(int i=0; i<k; i++){
        ...
        fprintf(output_file, "%d th CS Request at %s by thread %d\n", i+1, s.c_str(), id);//printing request time

        auto time1 = chrono::high_resolution_clock::now();
        while(lock.test_and_set())//spinning around lock
            ;
        ...
        fprintf(output_file, "%d th CS Exit at %s by thread %d\n", i+1, s.c_str(), id);//printing exit time
        lock.clear();//clearing lock, critical section ends here
...
int main()
{
    ...
    fprintf(output_file, "TAS ME Output:\n");
    thread threads[n];
    for(int i=0; i<n; i++){
