...
bool *waiting; //waiting queue(to ensure fairness)
...
void testCS(int id)
{
    ...
    for(int i=0; i<k; i++){
        ...
        waiting[id] = true;
        int key = 1;
        while (waiting[id] && key==1)//spinning around lock
        {
            int a = 0, b = 1;
        ...
        //giving lock to next thread in the waiting queue
        int m = (id+1)%n;
        while((m!=id) && !waiting[m]){
            m = (m+1)%n;
        }
        if(m==id){
            lock=0;
